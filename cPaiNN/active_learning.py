...
from cPaiNN.data import collate_atomsdata
...
class FeatureStatistics:
    ...
    def _compute_ens_stats(self, model_inputs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """Compute energy variance, forces variance, energy absolute error, and forces absolute error"""
        results_dict = self.ensemble(model_inputs)
        ens_stats = {}
...
#                global_g.append(torch.cat(model_g))
    ...
    def get_ens_stats(self, dataset: Optional[torch.utils.data.Dataset]=None) -> Dict[str, torch.Tensor]:
        ...
        if self.ens_stats is None:
            if self.ensemble is None:
                self.ensemble =EnsembleCalculator(self.models)
            dataloader = torch.utils.data.DataLoader(
                dataset=dataset,
                batch_size=self.batch_size,
                collate_fn=collate_atomsdata,
            ...
            for batch in dataloader:
                batch = {k: v.to(self.device) for k, v in batch.items()}
                ens_stats.append(self._compute_ens_stats(batch))
    ...
    def get_F_inv(self) -> torch.Tensor:
        ...
        if self.F_reg_inv is None:
            g = self.get_features()
            F = self.get_fisher().squeeze()
            # empirical regularisation
            lam = torch.trace(F) / (g.shape[1] * g.shape[2])
...
class GeneralActiveLearning:
    ...
    def select(
        ...
        elif self.selection == 'lcmd_greedy':                # lcmd is pool + train based
    ...
    def _get_kernel_matrix(self, pool_stats: FeatureStatistics, train_stats: Optional[FeatureStatistics]=None) -> KernelMatrix:
        ...
        if self.kernel == 'full-g':
            ...
            return DiagonalKernelMatrix(pool_stats.get_ens_stats()['e_var'])
        elif self.kernel == 'qbc-force':
            return DiagonalKernelMatrix(pool_stats.get_ens_stats()['f_var'])
        elif self.kernel == 'ae-energy':
            return DiagonalKernelMatrix(pool_stats.get_ens_stats()['e_ae'])
        elif self.kernel == 'ae-force':
            return DiagonalKernelMatrix(pool_stats.get_ens_stats()['f_ae'])
