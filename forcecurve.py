...
# forcecurve.py from ASE package, rewritten, ref: https://wiki.fysik.dtu.dk/ase/ase/neb.html
...
class ForceFit(namedtuple('ForceFit', ['path', 'energies', 'fit_path',
    ...
    def plot(self,c,label, ax=None, show_forces=True, show_fit=True,**kwargs):
        import matplotlib.pyplot as plt
        if show_fit:
            if ax is None:
                ax = plt.gca()
            ax.plot(self.path, self.energies, 'o',color=c)
            if show_forces:
                for x, y in self.lines:
                    ax.plot(x, y, '-g')
            ax.plot(self.fit_path, self.fit_energies,'-',color=c,label=label,**kwargs)
